import changeStatusAndPriority from "./changeStatusAndPriority.js";
...
function renderTask() {
	...
	//I decided to go with a class because in essence all the tasks have identical properties
	class Task {
		//Defines all the necessary properties
		constructor(desc, deadline, status, priority, parentSelector) {
			this.desc = desc;
			this.deadline = `${deadline}`.slice(0, 10);
			this.status = status;
			this.priority = priority;
			this.parent = document.querySelector(parentSelector);
		}
		render() {
			//Creares a new elemnt, which will be filled with information, and then appended to the DOM tree
			const element = document.createElement("div");
			//These variable store the CSS class for additional styling
			let statusClass;
			let priorityClass;
			//Depending on the task status a CSS class is selected
			switch (this.status) {
				case "Todo":
					statusClass = "todo";
					break;
			...
			//Depending on the task priority a CSS class is selected
			...
			//Adds a class for styling to the previously created element
			element.classList.add("render__item");
			//Sets the innerHTML of the previously created element
			element.innerHTML = `
				<div>
					<div class="render__item__desc active">${this.desc}</div>
					<input type="text" class="render__item__text">
				</div>
				<div>
					<div class="render__item__deadline active">${this.deadline}</div>
					<input type="date" class="render__item__date">
				</div>
				<div class="render__item__status ${statusClass}">${this.status}</div>
				<div class="render__item__priority ${priorityClass}">${this.priority}</div>
				<div class="render__item__icons">
					<div>
						<svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
						viewBox="0 0 512 512" style="enable-background:new 0 0 512 512;" xml:space="preserve" class="render__item__icons__pencil active">
							<g>
						...
						<div class="render__item__icons__yn-edit">
							<img src="./icons/green-check.svg" alt="Confirm" class="render__item__icons__confirm-edit">
							<img src="./icons/x.png" alt="Cancel" class="render__item__icons__cancel-edit">
						</div>
					</div>
					<div>
						...
						style="enable-background:new 0 0 612 612;" xml:space="preserve" class="render__item__icons__trash active">
							<g>
						...
						<div class="render__item__icons__yn-delete">
							<img src="./icons/green-check.svg" alt="Confirm" class="render__item__icons__confirm-delete">
							<img src="./icons/x.png" alt="Cancel" class="render__item__icons__cancel-delete">
						</div>
					</div>
				</div>`;
			//Appends the element to the DOM tree
			this.parent.append(element);
		}
	}
	//Removes all previously created elements and event listeners assigned to them, essentialy doing a rerender of the task list, inspired by React
	document.querySelectorAll(".render__item").forEach((a) => a.remove());
	//Renders all tasks from the array
	tasksArr.forEach(({desc, deadline, status, priority}) => {
		new Task(desc, deadline, status, priority, ".render__list").render();
	});
	//These three function calls assign all the necessary event listeners to their specific elements
